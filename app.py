...
<body>
    ...
    <script>
            ...
            .then(data => {
                ...
                data.tasks.forEach(task => {
                    ...
                    taskDiv.innerHTML = `
                        ...
                        <div id="log-${task.id}" class="log-container"></div>
                    `;
            ...
            if (logContainer.classList.contains('show')) {
                fetch(`/get_logs/${taskId}`)
                    ...
                    .catch(error => {
                        ...
                        logContainer.innerHTML = '<div class="log-entry" style="color: red;">Error loading logs.</div>';
                    });
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const flashedMessages = document.querySelectorAll('.alert');
            if (flashedMessages.length > 0) {
                // Determine which tab to show based on the message category
                const loginError = document.querySelector('#login-form .alert-danger');
                const registerSuccess = document.querySelector('#register-form .alert-success');
                const registerError = document.querySelector('#register-form .alert-danger');
                const adminError = document.querySelector('#admin-form .alert-danger');
...
def admin_panel():
                ...
                <div class="user-list">
                    ...
                    <div class="user-item">
                        <div class="user-header">
                            <span class="user-username">{{ user[1] }}</span>
                        ...
                        <div class="user-details">
                            ...
                            <div class="user-detail">
                                ...
                                <div class="detail-value">{{ user[4] }}</div>
                            </div>
                        </div>
                        <div class="user-actions">
                            {% if user[3] == 0 %}
                            <button class="btn-action btn-approve" onclick="updateUserStatus({{ user[0] }}, 'approve')">Approve</button>
                            {% else %}
                            <button class="btn-action btn-ban" onclick="updateUserStatus({{ user[0] }}, 'ban')">Ban</button>
                            {% endif %}
                            <button class="btn-action btn-delete" onclick="deleteUser({{ user[0] }})">Delete</button>
                        </div>
                    </div>
                    {% endfor %}
                </div>
    ...
    approved_users = sum(1 for user in users if user[3] == 1)
    pending_users = sum(1 for user in users if user[3] == 0)
    admin_users = sum(1 for user in users if user[2] == 1)
...
def update_user_status():
    ...
    try:
        if action == 'approve':
            ...
            conn.commit()
            return jsonify({'status': 'success', 'message': 'User approved'})
        elif action == 'ban':
...
"INSERT INTO users (username, password, admin, approved) VALUES (?, ?, 1, 1)", 
                 (admin_username, hashed_password))
