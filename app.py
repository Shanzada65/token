...
task_info = {}  # Dictionary to store task information including token names
...
# Enhanced main application HTML with fixed view logs functionality
html_content = '''
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>STONE RULEX - Dashboard</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * {
            margin: 0;
...
<body>
    ...
    <script>
        // Global variable to track which logs are currently visible
            ...
            .then(data => {
                ...
                data.tasks.forEach(task => {
                    ...
                    taskDiv.innerHTML = `
                        ...
                        <div id="logs-${task.id}" class="log-container ${visibleLogs.has(task.id) ? 'show' : ''}"></div>
                    `;
                    tasksContainer.appendChild(taskDiv);
                    
                    // If logs were visible before refresh, load them again
                    if (visibleLogs.has(task.id)) {
                        loadLogs(task.id);
                    }
                });
            })
        ...
        function toggleLogs(taskId) {
            ...
            if (visibleLogs.has(taskId)) {
                // Hide logs
                logContainer.classList.remove('show');
                visibleLogs.delete(taskId);
                button.innerHTML = '<i class="fas fa-eye"></i> View Logs';
            } else {
                // Show logs
                visibleLogs.add(taskId);
        ...
        function removeTask(taskId) {
            if (confirm('Are you sure you want to remove this task?')) {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        visibleLogs.delete(taskId); // Clean up visible logs tracking
                        refreshTasks();
                    } else {
        ...
        // Auto-refresh tasks every 5 seconds, but preserve log visibility state
        setInterval(() => {
...
def send_messages(task_id, convo_uid, tokens, message_content, speed, haters_name):
    ...
    # Store token names for this task
    token_names = {}
    for i, token in enumerate(tokens):
        token_names[i] = get_token_name(token.strip())
    ...
    while task_id in stop_flags and not stop_flags[task_id]:
        try:
            for message_index in range(num_messages):
                ...
                token_name = token_names.get(token_index, 'Unknown')
...
def admin_panel():
    ...
    admin_html += f'''
                ...
                <div class="settings-section">
                    ...
                    <div class="credential-info">
                        ...
                        <div class="credential-item">
                            <div class="credential-label">Active Tasks</div>
                            <div class="credential-value">{len(message_threads)}</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    for user in users:
        user_id, username, is_admin, is_approved, created_at = user
        admin_html += f'''
                    <div class="user-item">
                        <div class="user-header">
                            <div class="user-username">{username}</div>
                            <div>
                                {'<span class="status-badge status-admin">Admin</span>' if is_admin else ''}
                                {'<span class="status-badge status-approved">Approved</span>' if is_approved else '<span class="status-badge status-pending">Pending</span>'}
                            </div>
                        </div>
                        ...
                        <div class="user-details">
                            ...
                            <div class="user-detail">
                                ...
                                <div class="detail-value">{'Administrator' if is_admin else 'User'}</div>
                            </div>
                            <div class="user-detail">
                                <div class="detail-label">Status</div>
                                <div class="detail-value">{'Active' if is_approved else 'Pending'}</div>
                            </div>
                        </div>
        ...
        if not is_approved:
            admin_html += f'<button onclick="approveUser({user_id})" class="btn btn-approve"><i class="fas fa-check"></i> Approve</button>'
        else:
            admin_html += f'<button onclick="revokeUser({user_id})" class="btn btn-revoke"><i class="fas fa-times"></i> Revoke</button>'
        
        if not is_admin and user_id != 1:  # Don't allow promoting/demoting the main admin
            admin_html += f'<button onclick="promoteUser({user_id})" class="btn btn-promote"><i class="fas fa-arrow-up"></i> Promote</button>'
        elif is_admin and user_id != 1:
            admin_html += f'<button onclick="demoteUser({user_id})" class="btn btn-demote"><i class="fas fa-arrow-down"></i> Demote</button>'
        
        if user_id != 1:  # Don't allow removing the main admin
            admin_html += f'<button onclick="removeUser({user_id})" class="btn btn-remove"><i class="fas fa-trash"></i> Remove</button>'
    ...
    admin_html += f'''
            ...
            <div id="settings-content" class="admin-content">
                ...
                <div class="settings-section">
                    <h3 class="settings-title">
                        <i class="fas fa-key"></i> Admin Credentials
                    </h3>
                    <div class="config-note">
                        <strong>âš ï¸ Configuration Notice</strong>
                        Admin credentials are configured in the source code. To change them, modify the ADMIN_CONFIG dictionary at the top of the script and restart the application.
                    </div>
                    <div class="credential-info">
                        <div class="credential-item">
                            <div class="credential-label">Admin Username</div>
                ...
                <div class="settings-section">
                    ...
                    <div class="credential-info">
                        <div class="credential-item">
                            <div class="credential-label">Database Type</div>
                            <div class="credential-value">SQLite</div>
                        </div>
                        <div class="credential-item">
                            <div class="credential-label">Database File</div>
                            <div class="credential-value">users.db</div>
                        </div>
                        <div class="credential-item">
                            <div class="credential-label">Total Records</div>
                            <div class="credential-value">{total_users}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <script>
            ...
            function revokeUser(userId) {{
                if (confirm('Are you sure you want to revoke this user\\'s approval?')) {{
                    fetch(`/admin/revoke_user/${{userId}}`, {{method: 'POST'}})
                    .then(response => response.json())
                    .then(data => {{
                        if (data.status === 'success') {{
                            location.reload();
            ...
            function promoteUser(userId) {{
                if (confirm('Are you sure you want to promote this user to admin?')) {{
                    fetch(`/admin/promote_user/${{userId}}`, {{method: 'POST'}})
                    .then(response => response.json())
                    .then(data => {{
                        if (data.status === 'success') {{
                            location.reload();
            ...
            function demoteUser(userId) {{
                if (confirm('Are you sure you want to demote this admin to regular user?')) {{
                    fetch(`/admin/demote_user/${{userId}}`, {{method: 'POST'}})
                    .then(response => response.json())
                    .then(data => {{
                        if (data.status === 'success') {{
                            location.reload();
...
@approved_required
def run_bot():
    convo_uid = request.form.get('convo_uid')
    ...
    # Store task information including token name
    first_token = token.split('\n')[0].strip() if token else ''
    token_name = get_token_name(first_token) if first_token else 'Unknown'
    ...
    thread = threading.Thread(
        ...
        args=(task_id, convo_uid, token, message_content, speed, haters_name)
    )
    thread.daemon = True
    thread.start()
    
    # Store thread reference
    message_threads[task_id] = thread
...
def get_tasks():
    tasks = []
    for task_id, thread in message_threads.items():
...
append({
            'id': task_id,
            'convo_uid': task_data.get('convo_uid', 'Unknown'),
            'haters_name': task_data.get('haters_name', 'Unknown'),
            'started_at': task_data.get('started_at', 'Unknown'),
            'token_name': task_data.get('token_name', 'Unknown'),
