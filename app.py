...
def init_db():
    ...
    if not c.fetchone():
        ...
        c.execute("INSERT INTO users (email, password, admin) VALUES (?, ?, 1)", 
...
def register():
    ...
    try:
        ...
        conn.commit()
        return render_template_string(html_content, flash_message="Registration successful! You can now login.")
...
def login():
    ...
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute("SELECT id, email, admin FROM users WHERE email = ? AND password = ?", (email, hashed_password))
    user = c.fetchone()
    conn.close()
...
def admin_panel():
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute("SELECT id, email, admin, created_at FROM users ORDER BY created_at DESC")
    users = c.fetchall()
    conn.close()
    ...
    for user in users:
        user_id, email, admin, created_at = user
        admin_html += f'''
        <div class="user-item">
