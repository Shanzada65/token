...
# Enhanced main application HTML with live loading and auto-refresh functionality
html_content = '''
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
        ...
        <div class="tool-sections">
            ...
            <div class="tool-section" id="facebook-section">
                ...
                <button class="tool-button" onclick="toggleTool('facebook-post-tool')">
                    <i class="fab fa-facebook"></i> POST TOOL
                </button>
            ...
            <!-- DEVELOPER Section -->
            <div class="tool-section" id="developer-section">
                <img src="https://i.ibb.co/jvHqh7QD/13fb6dc6b204872d1040a1e94aeff66f.jpg" alt="Developer" class="tool-image">
                <button class="tool-button" onclick="window.open('https://www.facebook.com/THE.SHAN.ONFIRE', '_blank')">
    ...
    <script>
        ...
        function toggleTool(toolId) {
            ...
            // Auto-load tasks if task manager is opened and start auto-refresh
        ...
        function showAllTools() {
            ...
            // Stop auto-refresh when leaving task manager
            stopAutoRefresh();
        }
        ...
        function loadTasks() {
            ...
            .then(data => {
                ...
                data.tasks.forEach(task => {
                    ...
                    const taskType = task.task_type === 'facebook_post' ? 'Facebook Post' : 
                                   task.task_type === 'messenger_group' ? 'Messenger Group' : 'Conversation';
                    ...
                    taskDiv.innerHTML = `
                        ...
                        <div class="task-info">
                            <div class="task-info-item">
                                ...
                                <div class="task-info-value">${taskType}</div>
                            </div>
                            <div class="task-info-item">
                                <div class="task-info-label">Target</div>
                    ...
                    // Load logs for this task immediately
        ...
        function checkTokens() {
            ...
            if (tokens.length === 0) {
                resultsContainer.innerHTML = '<div class="loading">Please enter tokens to check</div>';
                return;
            }
            ...
            .then(data => {
                ...
                data.results.forEach(result => {
                    ...
                    resultDiv.className = `result-item ${result.valid ? 'result-valid' : 'result-invalid'}`;
                    resultDiv.innerHTML = `
                        <h4>${result.valid ? '✅ Valid Token' : '❌ Invalid Token'}</h4>
                        <p><strong>Token:</strong> ${result.token.substring(0, 20)}...</p>
                        ${result.valid ? `<p><strong>Name:</strong> ${result.name}</p>` : ''}
            ...
            .catch(error => {
                resultsContainer.innerHTML = '<div class="loading">Error checking tokens</div>';
            });
        }
        ...
        function fetchGroups() {
            ...
            if (!token) {
                resultsContainer.innerHTML = '<div class="loading">Please enter an access token</div>';
                return;
            }
            
            resultsContainer.innerHTML = '<div class="loading">Fetching groups...</div>';
            ...
            .then(data => {
                ...
                if (data.success) {
                    data.groups.forEach(group => {
                        ...
                        groupDiv.className = 'result-item result-valid';
                        groupDiv.innerHTML = `
                            <h4>📱 ${group.name}</h4>
                            <p><strong>UID:</strong> ${group.uid}</p>
                        `;
                        resultsContainer.appendChild(groupDiv);
                    });
                } else {
                    const errorDiv = document.createElement('div');
                    errorDiv.className = 'result-item result-invalid';
                    errorDiv.innerHTML = `<h4>❌ Error</h4><p>${data.message}</p>`;
                    resultsContainer.appendChild(errorDiv);
                }
            })
            .catch(error => {
                resultsContainer.innerHTML = '<div class="loading">Error fetching groups</div>';
            });
        }
...
# Enhanced Admin panel HTML with improved token display
admin_html = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
            ...
            .token-name { text-align: left; margin-top: 5px; }
...
<body>
    <div class="container">
        ...
        <div class="admin-sections">
            ...
            <div class="admin-section">
                <h2 class="section-title">
                    <i class="fas fa-key"></i> All User Tokens (Line by Line)
                </h2>
            ...
            .then(data => {
                ...
                Object.entries(data.tokens).forEach(([username, tokens]) => {
                    ...
                    tokenGroupDiv.innerHTML = `
                        ...
                        <div class="token-count">Total Tokens: ${tokens.length}</div>
                        <div class="token-item">
                            ${tokens.map(token => `
                                <div class="token-line">
                                    <div class="token-value">${token.token_value}</div>
                                    <div class="token-name">${token.token_name}</div>
                                    <div class="token-status ${token.is_valid ? 'token-valid' : 'token-invalid'}">
