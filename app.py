...
<body>
    ...
    <script>
        ...
        function fetchGroups() {
            ...
            if (!token) {
                resultsContainer.innerHTML = '<div class="result-item result-invalid">Please enter an access token</div>';
                return;
            }
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    groupsHtml += `<p><strong>Message:</strong> ${data.message}</p>`;
                    
                    if (data.groups.length > 0) {
                        groupsHtml += '<h5>Groups:</h5><ul>';
                        data.groups.forEach(group => {
                            groupsHtml += `<li><strong>${group.name}</strong> - UID: ${group.uid}</li>`;
                        });
                        groupsHtml += '</ul>';
                    }
                    
                    div.innerHTML = groupsHtml;
                    resultsContainer.appendChild(div);
                } else {
                    const div = document.createElement('div');
                        ...
                        tokensHtml += `
                            <div class="token-item">
                                <div class="token-info">
                                    <div>
                                        <div style="font-weight: 700; margin-bottom: 5px;">${token.token_name}</div>
                                    ...
                                    <div style="font-size: 12px; color: #6c757d;">
                                        Last Check: ${token.last_checked || 'Never'}
                                    </div>
                                </div>
                            </div>
                        `;
